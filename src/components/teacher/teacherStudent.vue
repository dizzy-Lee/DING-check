<template>
  <div class="student-main" ref="student_main">
    ...
    <div class="header dd-border-bottom" ref="header_top_1">
            <span style="padding:1rem;line-height:1.5rem;position:absolute;left:0;margin-left:0.5rem;" class="iconfont" @click="blackHome">&#xe600;</span>
      <span class="title">作业评价</span>
      <span style="position:absolute;right:1rem;line-height:3.5rem;font-family: PingFangSC-Regular;font-size: 16px;color: #212121;text-align: center;" @click="handleShowAssess">编辑</span>
    </div>
    ...
    <div style="position:absolute;overflow: hidden;top:3.7rem;bottom:0rem;margin:0 auto;width: 100%;" ref="main_wrapper_1">
      <div style="margin:0 auto;">
        <div class="reception-time">
          <div class="time">{{this.$store.state.teacher_Student_Detail.date}}</div>
          <div class="reception-count">第 <span>{{this.$store.state.teacher_Student_Detail.frequency}}</span> 次验收</div>
        </div>
        ...
        <div class="reception-detail">
          ...
          <div class="null-div dd-border-bottom">
            <div class="user-img-name">
              <!-- :src="this.$store.state.teacher_Student_Detail.user_img"  这是头像 -->
              <img :src="this.$store.state.teacher_Student_Detail.img" alt="">
              <div>{{this.$store.state.teacher_Student_Detail.name}}</div>
            </div>
          </div>
          <!-- 评价 -->
          <div class="pingjia-detail dd-border-bottom">
            <!-- 评价等级图  ../../../static/img/超神.png-->
            <img :src="isSrcType(this.$store.state.teacher_Student_Detail.evaluate)" alt="">
            <!-- 评价名称 -->
            <span class="text" :style="{color:typeColor(this.$store.state.teacher_Student_Detail.evaluate)}" v-if="this.$store.state.teacher_Student_Detail.acceptance!='0'">{{this.$store.state.teacher_Student_Detail.evaluate}}</span>
            <!-- 如果是未验收则显示这一行-->
            <span v-if="this.$store.state.teacher_Student_Detail.acceptance=='0'" class="no_acceptance">暂未验收</span>
          </div>
          <!-- 作业标签  先判断是不是未验收-->
          <!-- <div class="homework-label" v-if="this.$store.state.studentHomeShow.is_acceptance"> -->
               <div class="homework-label" v-if="this.$store.state.teacher_Student_Detail.acceptance!='0'">
            <div class="homework-label">
            <div class="title" >作业标签</div>
            <div class="label-detail">
              <div class="label-item" v-for="(item,index) in this.$store.state.teacher_Student_Detail.quickEvaluate" :key="index">
                {{item}}
              </div>
            </div>
          </div>
          ...
          <div class="tutor-comment" >
            <div class="title" v-if="this.$store.state.teacher_Student_Detail.acceptance!='0'">助教简评</div>
            <p v-if="this.$store.state.teacher_Student_Detail.acceptance!='0'">{{this.$store.state.teacher_Student_Detail.otherEvaluate}}</p>
            <!-- 如果未验收 -->
            <p v-if="this.$store.state.teacher_Student_Detail.acceptance=='0'" class="no-tutor-acceptance">你的助教已经饥渴难耐快去找他验收吧！</p>
          </div>
          <!-- 查看历史 -->
        </div>
      <!-- </div> -->
...
export default {
  ...
  methods: {
    ...
    isSrcType(type) {
      switch (type) {
        ...
        case "大坑":
          ...
          break;
        case "未交":
         if(this.$store.state.teacher_Student_Detail.acceptance=='0'){
    ...
    blackHome(){
      this.$parent.$data.isShowTeacherStudent = false;
    },
    handleShowAssess(){
      this.$parent.$data.isShowAssess = true;
    }
  },
  created() {
