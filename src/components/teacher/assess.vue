<template>
  ...
  <div class="home">
    ...
    <div class="home_avatar dd-border-bottom">
      <v-avatar size="large" :src="this.$store.state.teacher_Student_Detail.img"></v-avatar>
      <p>{{this.$store.state.teacher_Student_Detail.name}}</p>
...
export default {
  ...
  methods: {
    goAcceptdetails() {
      this.$parent.$data.isShowAssess = false;
    },
    ...
    pushAssess: function() {
      ...
      axios
        .post("/api/teacher-ins-submit", {
          ...
          count: String(this.$store.state.teacher_Student_Detail.frequency)
          ...
          if (res.data == true) {
            ...
            axios
              .post("/api/teacher-load-click", {
                ...
                count: this.$store.state.teacherDetail[1]
              })
              .then(res => {
            ...
            // this.$parent.$data.isShowTeacherDetail=false;     // 这两个直接返回到主页面
          }
        })
        .catch(error => {
          console.log(error);
